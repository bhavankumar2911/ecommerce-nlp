...
def get_product_links (product):
    ...
    url = "https://www.flipkart.com/search?q={}&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off".format(encoded)
...
async def get_all_product_reviews(product_links):
    tasks = [asyncio.ensure_future(get_one_product_reviews(product_link)) for product_link in product_links]
    ...
    for page in product_pages:
        ...
        reviews_link = reviews_link.get("href")
        links_of_review_pages.append("https://www.flipkart.com{}".format(reviews_link))
...
async def get_reviews_from_review_pages(links_of_review_pages):
    tasks = [asyncio.ensure_future(get_one_product_reviews(link)) for link in links_of_review_pages]
...
loop = asyncio.new_event_loop()
asyncio.set_event_loop(loop)
loop.run_until_complete(get_reviews_from_review_pages(links_of_review_pages))
...
sorted_products = sorted(link_and_ratings, key=lambda i: i['positive'], reverse=True)
